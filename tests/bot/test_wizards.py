...
from src.bot.handlers import wizard_close_position, wizard_market_order
...
class TestMarketWizardCallbackParsing:
    ...
    async def test_market_coin_selected_parsing(self):
        ...
        await wizard_market_order.market_coin_selected(update, context)
    ...
    async def test_market_side_selected_parsing_buy(self):
        ...
        await wizard_market_order.market_side_selected(update, context)
    ...
    async def test_market_amount_selected_parsing(self):
        ...
        with patch("src.bot.handlers.wizard_market_order.convert_usd_to_coin") as mock_convert:
            mock_convert.return_value = (
                0.00096,
                104088.0,
            )  # (coin_size, current_price)

            await wizard_market_order.market_amount_selected(update, context)
    ...
    async def test_close_position_callback_parsing(self):
        ...
        with patch("src.bot.handlers.wizard_close_position.position_service") as mock_pos_service:
            ...
            await wizard_close_position.close_position_selected(update, context)
    ...
    def test_callback_data_format_validation(self):
        ...
        """
        # Valid formats
        assert ["side_buy", "BTC"] == ["side_buy", "BTC"]
        assert ["side_sell", "ETH"] == ["side_sell", "ETH"]
        assert ["select_coin", "SOL"] == ["select_coin", "SOL"]
...
class TestMarketAmountErrorHandling:
    ...
    async def test_market_amount_invalid_raises_error(self):
        ...
        result = await wizard_market_order.market_amount_selected(update, context)
...
class TestCustomAmountFlow:
    ...
    async def test_market_custom_amount_request(self):
        ...
        from src.bot.handlers.wizard_market_order import MARKET_AMOUNT

        result = await wizard_market_order.market_custom_amount(update, context)
...
class TestMarketExecution:
    ...
    async def test_market_execute_success(self):
        ...
        with patch("src.bot.handlers.wizard_market_order.place_order_use_case") as mock_use_case:
            ...
            result = await wizard_market_order.market_execute(update, context)
...
class TestWizardCancellation:
    ...
    async def test_cancel_with_callback_query(self):
        ...
        context = TelegramMockFactory.create_context(
            user_data={"market_coin": "BTC", "market_is_buy": True}
        )

        result = await wizard_market_order.wizard_cancel(update, context)
...
class TestClosePositionHandlers:
    ...
    async def test_close_position_execute_uses_size_closed(self):
        ...
        with patch(
            "src.bot.handlers.wizard_close_position.close_position_use_case"
        ) as mock_use_case:
            ...
            await wizard_close_position.close_position_execute(update, context)
    ...
    async def test_close_position_selected_value_error(self):
        ...
        with patch("src.bot.handlers.wizard_close_position.position_service") as mock_service:
...
class TestMarketWizardStart:
    ...
    async def test_market_wizard_start(self):
        ...
        from src.bot.handlers.wizard_market_order import MARKET_COIN

        result = await wizard_market_order.market_wizard_start(update, context)
...
class TestMarketAmountTextInputPriceError:
    ...
    async def test_market_amount_text_input_runtime_error(self):
        ...
        with patch("src.bot.handlers.wizard_market_order.convert_usd_to_coin") as mock_convert:
            ...
            assert result == wizard_market_order.ConversationHandler.END
            # reply_text called twice: once for loading message, once for error via send_error_and_end()
            assert update.message.reply_text.call_count == 2
            # Verify second call is the error message with main menu
