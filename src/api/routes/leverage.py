...
class ValidateLeverageRequest(BaseModel):
    ...
    coin: Optional[str] = Field(None, description="Optional coin for coin-specific checks")
...
def set_coin_leverage(request: SetLeverageRequest):
    ...
    try:
        logger.info(
            f"Setting leverage for {request.coin}: "
            f"{request.leverage}x (cross={request.is_cross})"
        )
